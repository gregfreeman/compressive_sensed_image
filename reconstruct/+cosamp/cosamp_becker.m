...
%       .cg_tol     = when "A" is a set of function handles, this controls
%                       the tolerance in the iterative solver 
%       .cg_maxit   = maximum number of steps in the iterative solver
...
%         x_T     = pinv( A(:,T) )*RHS;
    ...
    if HSS
        ...
        if TWO_SOLVES
            if LARGESCALE
                ...
                [x_T2,~] = LSQR_ALG(b,Afcn,x(ind_k)); % not using "r", just using "b"
            else
                x_T2     = A(:,ind_k)\b;
            end
            x( ind_k )  = x_T2;
    ...
    if ~isempty(errFcn)
        er  = errFcn(x);
        if PRINT, fprintf('%4d, %4d, %.2e, %.2e\n', kk, length(T), normR, er ); end
        errHist(kk)     = er;
    else
        if PRINT, fprintf('%4d, %4d, %.2e\n', kk, length(T), normR ); end
    end
    residHist(kk)   = normR;
