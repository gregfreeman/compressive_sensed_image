function [Sest, iterations, error] = cosamp(Phi,u,K,tol,maxiterations)
...
%       iterations: the number of iterations before tolerance reached
%       error: the error figure
%
...
while (t <= maxiterations) && (error > tol)
    disp(sprintf('Iteration %d of %d max iterations. error:%f',t,maxiterations,error));
  y = abs(Phi'*v);
